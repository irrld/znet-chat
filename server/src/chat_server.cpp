...
bool ChatServer::OnServerClientDisconnectedEvent(
  ...
  Ref<UserDisconnectedPacket> packet = CreateRef<UserDisconnectedPacket>();
  packet->user_id_ = user_id;
...
bool ChatServer::OnUserAuthorizedEvent(UserAuthorizedEvent& event) {
  ...
  user_list_[user->user_id()] = user->username();
  Ref<UserConnectedPacket> packet = CreateRef<UserConnectedPacket>();
  packet->user_id_ = user->user_id();
  packet->username_ = user->username();
  BroadcastPacket(packet);
  return false;
}
